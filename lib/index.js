...
function storeManager(session) {
	class SequelizeSessionStore extends session.Store {
		constructor(config) {
			...
			this._expirationInterval = config.expiration ? config.expiration.interval : 1000 * 60 * 60;
			this._sessionLife = config.expiration ? config.expiration.life : 1000 * 60 * 60 * 24;
